...
class use_emul:
    ...
    def filter_print(self, string, **kwargs):
        """Print function that only prints strings that are not in self.printed_messages."""
        if not (self.is_spam_filtered and string in self.printed_messages):
            print(string, **kwargs)
            self.printed_messages.add(string)
        ...
        if z in [0,0.5,1,1.5,2]:
            ...
            if z > 2:
                if str(self.above_zmax).lower() == 'zmax':
                    ...
                    self.filter_print('Redshift z>2 is outside the training set and therefore extrapolated.')
